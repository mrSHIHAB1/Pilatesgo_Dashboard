...
export default function SendEmailModal({ isOpen, user, onClose, onSent }: Props) {
  ...
  const canSend = useMemo(() => {
    return toEmail.trim().length > 0 && title.trim().length > 0 && message.trim().length > 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-400">Send Notification/Email</h2>
          <button
            onClick={onClose}
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-6">
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between rounded-2xl bg-pink-50/40 p-1 text-[11px] font-bold text-gray-400">
                <div className="flex-1 py-2 rounded-xl text-center">Push</div>
                <div className="flex-1 py-2 rounded-xl bg-[#FF87B2] text-white text-center">Email</div>
                <div className="flex-1 py-2 rounded-xl text-center">Both</div>
              </div>

              <div className="mt-5">
                <label className="block text-[11px] font-bold text-gray-500 mb-2">To</label>
                <input
                  value={toEmail}
